...
class ChecklistTemplate(Base):
    ...
    name = Column(String(255), nullable=False)
    description = Column(Text, nullable=True)
    equipment_type = Column(String(100), nullable=True)  # Camera, NVR, Router, etc.
    security_type = Column(String(50), nullable=True)  # CCTV, Access Control, Alarm, Network
    is_active = Column(Boolean, default=True)
    created_at = Column(DateTime(timezone=True), default=lambda: datetime.now(timezone.utc))
    updated_at = Column(DateTime(timezone=True), default=lambda: datetime.now(timezone.utc))
...
class ChecklistTemplateItem(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    template_id = Column(UUID(as_uuid=True), ForeignKey("fsm.checklist_templates.id"), nullable=False)
    item_order = Column(Integer, nullable=False, default=1)
    question = Column(String(500), nullable=False)
    item_type = Column(String(20), default="checkbox")  # checkbox, text, number, photo, select
...
class VisitChecklist(Base):
    __tablename__ = "visit_checklists"
    __table_args__ = {"schema": "fsm"}
...
class VisitChecklistItem(Base):
    __tablename__ = "visit_checklist_items"
    __table_args__ = {"schema": "fsm"}

    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    checklist_id = Column(UUID(as_uuid=True), ForeignKey("fsm.visit_checklists.id"), nullable=False)
    template_item_id = Column(UUID(as_uuid=True), ForeignKey("fsm.checklist_template_items.id"), nullable=False)
    item_order = Column(Integer, nullable=False)
    question = Column(String(500), nullable=False)
    item_type = Column(String(20), default="checkbox")
    answer = Column(Text, nullable=True)  # checkbox: "yes"/"no", text: value, number: value
