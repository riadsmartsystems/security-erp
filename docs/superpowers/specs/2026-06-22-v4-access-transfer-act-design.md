...
### 2.2 Міграція `Vault Audit Log.action` Select

Поточний список (R8 + V2): `view / create / update / export / act_generate / mfa_fail`

V4 додає три значення: `act_revoke`, `act_view`, `act_acknowledge`.

Файл `doctype/vault_audit_log/vault_audit_log.json` — оновити `options` поля `action`:
```
\nview\ncreate\nupdate\nexport\nact_generate\nmfa_fail\nact_revoke\nact_view\nact_acknowledge
```

DoD: `bench migrate` пройшов чисто після оновлення json.
...
4. Якщо `delivery_token` вже є і `link_burned == 0` → спроба revoke старого:
   ```python
   old_token = frappe.cache().get_value(f"act:act_to_tok:{act_name}")
   if old_token:
       # старий токен ще живий у Redis — явно анулюємо
       frappe.cache().delete_value(f"act:tok:{old_token}")
       frappe.cache().delete_value(f"act:otp:{old_token}")
       append_audit_log("act_revoke", vault_entry="", field_touched=act_name, ...)
   else:
       # TTL Redis вже вийшов (>24h) — посилання протухло само по собі,
       # аудит act_revoke не потрібен, просто генеруємо нове
       pass
   ```
...
Повертає метадані акту без OTP і без decrypt. Дозволяє клієнту перевірити посилання до введення коду.

**Жорстка whitelist полів відповіді** (більше нічого не включати):
```python
return {
    "act_name": act.name,
    "passport_label": act.passport,           # назва об'єкта (не id)
    "customer_label": act.customer,           # назва клієнта (не id)
    "generated_at": str(act.generated_at),
    "expires_at": str(act.delivery_token_expires_at),
    "entry_count": len(act.included_entries),
    "link_burned": bool(act.link_burned),
}
```
Заборонено включати: `delivery_token`, `generated_by`, `audit_ref`, `included_entries` (деталі), будь-які `*_enc` поля.
...
| OTP brute-force | **Прийнятий ризик (задокументовано).** Теоретично 10^6 / 24h ≈ 694 спроби/хв. Але атака вимагає знання `token_hex` (64 hex = 256-bit секрет, не вгадується). Token — перша лінія захисту; OTP — другий незалежний фактор. Перебір OTP без знання token_hex неможливий. Rate-limit на публічний endpoint — відкладено до H2 як додаткова hardening-міра. |
...
7. `vault_audit_log.json` оновлено (нові action-значення); `bench migrate` пройшов чисто
8. Vault isolation linter (V2) — зелений після додавання `act.py`
9. BUILD_LOG оновлено; C2/H1 gate відмічено
